...
def solve_ass_symmetry_cpsat(graph: nx.Graph, time_limit: float = 60.0, num_colors: int = None) -> dict:
    if len(graph) == 0:
    ...
    for c in range(1, num_colors):
        model.Add(y[c] <= y[c - 1])
    for c in colors:
        model.Add(y[c] <= sum(x[v, c] for v in vertices))
...
def solve_ass_symmetry_gurobi(graph: nx.Graph, time_limit: float = 60.0, num_colors: int = None) -> dict:
    ...
    for c in colors:
        model.addConstr(y[c] <= gp.quicksum(x[v, c] for v in vertices))
